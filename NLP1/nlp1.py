...
DATA_DIR = r"C:\Users\Toprak\Documents\PythonProjeler\Ensemble Proje\NLP1"
    ...
    for i, data in enumerate(datasets[:1]):
        ...
        if isRandomForest:
            for max_depth in max_depths:
                RF = RandomForestClassifier(max_depth = max_depth, n_estimators = n_estimator,n_jobs=-1)
                RF.fit(X,y)
                estimators = RF.estimators_        
                # print(len(estimators))
                points = get_kappas(estimators,X,y)
            ...
            meta_meta_points.append(meta_points)
            plot_one_point2('Random Forest Sabit Tekil Öğrenici Maximum Derinliğe Göre Kappa - Error \n Train Dataset ',
            ...
            with open(str(data)+'sabitensemble_rf_meta_points.pkl', 'wb') as f:
                pickle.dump(meta_points, f)
            with open(str(data)+'sabitensemble_rf_meta_points2.pkl', 'wb') as f:
                pickle.dump(meta_points_2, f)    
            with open(str(data)+'sabitensemble_rf_score.pkl', 'wb') as f:
                pickle.dump(score, f)
            with open(str(data)+'sabitensemble_rf_score2.pkl', 'wb') as f:
